...
The default is %s and the available warnings are:
  ...
  29: cannot generate type-aware initializer for an unknown type
...
Supported options:|}
  ...
  *)
  ...
  if Options.wasm () && not (Options.debug "force-c") then
    ...
    let files = if !Options.hoist_locals then MergeDeclInit.merge_decl_init_files files else files in
